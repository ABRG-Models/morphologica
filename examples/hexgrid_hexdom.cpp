...
int main()
{
    ...
    // Create a HexGrid to show in the scene. Set it up with a base hexagonal domain
    morph::HexGrid hg(0.01, 2, 0, morph::HexDomainShape::Hexagon);
    ...
    for (unsigned int ri=0; ri<hg.num(); ++ri) {
        ...
        // To mark the boundary, set the dummy data 0 on the boundary
        if (hg.d_flags[ri] & HEX_IS_BOUNDARY) {
            data[ri] = 0.0f;
        }
    }
