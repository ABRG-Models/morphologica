...
int main()
{
    morph::Visual v(1600, 1000, "Demo of Grid showing a resampled image");
    v.setSceneTrans (morph::vec<float,3>({-2.60233f, 1.32237f, -6.5f}));
    ...
    morph::Grid g1(256U, 65U, dx, nul, morph::GridDomainWrap::Horizontal);

    // Resample onto a lower res grid
    morph::Grid g2(128U, 32U, dx2, nul, morph::GridDomainWrap::Horizontal);

    // Load an image
    std::string fn = "../examples/bike256_65.png";
    morph::vvec<float> image_data_bltr;
    morph::vec<unsigned int, 2> dims = morph::loadpng (fn, image_data_bltr, morph::vec<bool, 2>{false,true});
    ...
    morph::vvec<float> img_resampled = g2.resample_image (image_data_bltr, dims[0], image_scale, image_offset);
    ...
    morph::vvec<float> hex_image_data = hg.resampleImage (image_data_bltr, dims[0], image_scale, image_offset);
    ...
    gv1->setScalarData (&image_data_bltr);
    ...
    // Visualize resampled
    auto gv2 = std::make_unique<morph::GridVisual<float>>(&g2, morph::vec<float>({0,-2,0}));
    v.bindmodel (gv2);
    ...
    gv2->addLabel ("Resampled", {0, 0.2, 0}, morph::TextFeatures(0.1f));
    gv2->finalize();
    v.addVisualModel (gv2);

    auto hgv = std::make_unique<morph::HexGridVisual<float>>(&hg, morph::vec<float>({1,-4,0}));
    v.bindmodel (hgv);
    hgv->setScalarData (&hex_image_data);
    hgv->cm.setType (morph::ColourMapType::GreyscaleInv);
    hgv->zScale.setParams (0, 0);
    hgv->addLabel ("Resampled to hex", {-1, 0.5, 0}, morph::TextFeatures(0.1f));
    hgv->finalize();
