...
constexpr int gl_version = m::gl::version_4_6;  // modifying the default gl_version to use
...
struct VisualizerTemporalSignal
{
        ...
        nb_sample(std::ceil(time_window / sampling_time))
    {
        ...
        in_selected_sgv_ptr = std::make_unique<m::SimpleGridVisual<float, int, float, gl_version>>(
            ...
            "0b: selected pixels to display", offset_right, m::ColourMapType::RGB,
            grid_order_setting, 0.02f * img_h / 2
        );
        ...
        pixel_graph = std::make_unique<m::ConstantAbscissaGraphVisual<float, int, float,
                gl_version>>(
