...
struct Mnist2
{
    ...
    void init() {
        ...
        this->loadData ("train", this->training, this->training_f, this->training_label);
        this->loadData ("t10k", this->test, this->test_f, this->test_label);
    }

    void loadData (const std::string& tag,
                   std::vector<cv::Mat>& theMats,
                   std::vector<morph::vVector<float>>& vecFloats,
        ...
        for (int inum = 0; inum < n_imgs; ++inum) {
            ...
            vecFloats.push_back (ar);
            vecLabels.push_back (lbl);
        }
    }
    ...
    void showall() {
        ...
        for (unsigned int i = 0; i < this->training.size(); ++i) {
            cv::Mat m (this->nr, this->nc, CV_32F, cv::Scalar(0));
            this->training[i].copyTo (m);
            cv::imshow (winName, m);
            std::cout << "Label: " << (unsigned int)this->training_label[i] << std::endl;
            cv::waitKey(0);
        }
    }
    ...
    std::vector<morph::vVector<float>> training_f;
    //! The training label.
    std::vector<unsigned char> training_label;
    ...
    std::vector<morph::vVector<float>> test_f;
    //! The test/validation label.
