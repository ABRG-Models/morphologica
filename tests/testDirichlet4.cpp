...
int main()
{
    ...
    try {
        ...
        list<morph::DirichDom<float>> domains =
        ...
        auto pvp = std::make_unique<morph::PolygonVisual> (v.shaders, vec<float>({0,0,0}), centroid, centroidv, sz/16.0f, 0.01f, vec<float>({0,0,1}), 10);
        v.addVisualModel (pvp);
        // red hex at zero
        auto pvp2 = std::make_unique<morph::PolygonVisual> (v.shaders,vec<float>( {0,0,0.01f}), vec<float>({0,0,0}), vec<float>({0,1,0}), sz/20.0f, 0.01f, vec<float>({1,0,0}), 8);
        v.addVisualModel (pvp2);
