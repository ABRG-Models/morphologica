...
int main()
{
    ...
    try {
        ...
        vec<float,3> centroid = hg.boundaryCentroid.plus_one_dim();
        vec<float,3> centroidv = centroid + vec<float,3> ({ 0.0f, 1.0f, 0.0f });
        auto pvp = std::make_unique<morph::PolygonVisual> (vec<float>({0,0,0}), centroid, centroidv, sz/16.0f, 0.01f, vec<float>({0,0,1}), 10);
        v.bindmodel (pvp);
        pvp->finalize();
        v.addVisualModel (pvp);
        // red hex at zero
        auto pvp2 = std::make_unique<morph::PolygonVisual> (vec<float>( {0,0,0.01f}), vec<float>({0,0,0}), vec<float>({0,1,0}), sz/20.0f, 0.01f, vec<float>({1,0,0}), 8);
        v.bindmodel (pvp2);
        pvp2->finalize();
        v.addVisualModel (pvp2);
