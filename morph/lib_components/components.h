...
template<typename T, typename I = unsigned int, typename C = float,
    int gl_version = morph::gl::version_4_1>
...
private:
    ...
    {
        auto cgv =
            std::make_unique<m::GridVisual<float, int, float, gl_version>>(
            this->data_gd.get(), offset);
        v_ref.bindmodel(cgv);
        ...
        for (unsigned int y = pix_y; y < pix_y + selected_window_height; ++y) {
            for (unsigned int x = pix_x; x < pix_x + selected_window_width; ++x) {
                ...
                if (this->grid_order == m::GridOrder::topleft_to_bottomright) {
                    ...
                    std::cerr <<
                        "[ERROR in SimpleGridVisual] bottomleft_to_topright_colmaj and topleft_to_bottomright_colmaj grid orders are not supported yet"
                        << std::endl;
                }
...
public:
    ...
    ConstantAbscissaGraphVisual(
        m::Visual<gl_version> & v_ref, m::vec<float> graph_pos, float graph_width,
        float graph_height,
