...
/*
 * An Elman network, in which an input layer feeds forward to a hidden layer, which
 * feeds forward to an output layer, and also back to a context (or memory) layer, which
 * then feeds forward to the hidden layer.
 */
template <typename T>
struct ElmanNet
{
...
template <typename T>
std::ostream& operator<< (std::ostream& os, const ElmanNet<T>& ff)
{
    os << ff.str();
    return os;
}
