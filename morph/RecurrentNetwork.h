 ...
 */
    ...
    /*
        ...
        \brief A Neural Network model, capable of learning with an arbitrary network topology, with (attractor) activiation dynamics and (recurrent backpropagation) learning rule as described by Pineda F.J., (1987) Physical Review Letters, 59(19), p2229--2232.

        Feed-forward dynamics are given by (dx_i/dt)*tauX = -x_i + f(\sum_j x_j * W_ij)+I_i, where f(x)=1/(1+exp(-x)) is a sigmoid and I_i is the external input to input nodes.

        Feed-backward dynamics are given by (dy_i/dt)*tauY = -y_i + \sum_j(f_j' w_ji y_j) + J_i, where f_j'=f_j(x)*(1-f_j(x)) is the derivative of the sigmoid, and J_i=target_i-x_i is the discrepancy to be minimised

        Weight update is given by (dw_ij/dt)*tauW = x_i * y_j * f_j'.

        Individual connections (between pre and post units) need to be made by calling connect before the network can be properly initialized, so typical initialization would look like this:
...
public:
    ...
    void addBias(void){
        ...
        /*
            Add a bias input (i.e., threshold) to each node by simulating an extra node with a constant activation of 1.0, and modifiable connection weights from this node to all others. Optional.
        */
    ...
    void connect(int pre, int post){
        ...
        // adds a 0-weight connection between the pre-synaptic node identified by integer pre and the post-synaptic node identified by the integer post.
    ...
    void randomizeWeights(double weightMin, double weightMax){
        ...
        // set all network weights to random values from a uniform disribution in the range weightMin -- weightMax
    ...
    void setNet(void){
        ...
        // Register the number of connection weights and obtain pointers to them once all connections have been set.
   ...
   void forward(void){
    ...
    void setError(std::vector<int> oID, std::vector<double> targetOutput){
        ...
        // Compute the discrepancy between target and output values.
        // Supply the integer identity of the output nodes, and their corresponding double target values.
    ...
    void weightUpdate(void){
        ...
        /*
            ...
            Note that large weight updates are rejected, fixing an instability issue where weights (and thus error) jump to very large values as learning converges.
        */
    ...
    std::vector<double> getWeightMatrix(void){
        ...
        // returns a 1D vector of N**2 doubles (for saving) corresponding to the flattened NxN weight matrix
    ...
    bool convergeForward(void){
        ...
        // iteratively apply the feed-forward dynamics until either i) the dynamics have converged (sum_i (x_i(t)-x_i(t-1))^2<divergenceThreshold, or ii) a maximum number of settling steps has occurred. If i) return converged=true, else if ii) return converged=false
    ...
    bool convergeBackward(void){
        ...
        // iteratively apply the feed-backward dynamics until either i) the dynamics have converged (sum_i (y_i(t)-y_i(t-1))^2<divergenceThreshold, or ii) a maximum number of settling steps has occurred. If i) return converged=true, else if ii) return converged=false
   ...
   void convergeForward(double weightNudgeSize){
        ...
        // supply a weighNudgeSize to overload, adding a random 'nudge' of +/- weightNudgeSize to each weight when the forward dynamics fail to converge
    ...
    void convergeBackward(double weightNudgeSize){
        ...
        // supply a weighNudgeSize to overload, adding a random 'nudge' of +/- weightNudgeSize to each weight when the backward dynamics fail to converge
